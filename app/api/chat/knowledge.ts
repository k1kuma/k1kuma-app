// Knowledge Base for Matt Kikuchi
// This is the ONLY information the AI can reference about you
// Update this file to add/modify what the AI knows

export const knowledgeBase = `
# Matt Kikuchi - Professional Profile

## Current Role
- **Position**: Software Engineer
- **Company**: TrackInsight ETF (France-based startup)
- **Duration**: Currently employed
- **Focus**: FinTech and ETF data/analytics infrastructure

## Experience Summary
- **Total Experience**: 10+ years in software engineering
- **Domains**: FinTech, Web3, Data Engineering, Sports Analytics
- **Location**: Toronto, Canada (Born and raised in Canada)

## Background & Journey
- **Origin Story**: Got interested in programming through gaming as a kid
- **Pivotal Moment**: Interest in statistics for professional sports led to passion for data analysis
- **Evolution**: Transitioned from gaming interest → sports analytics → data engineering → FinTech/Web3
- **Education**: Computer Science at University of Western Ontario

## Professional Identity
- **What I Stand For**: "Building solutions to complex problems in the FinTech and web3 space"
- **Approach**: Solving complex technical challenges with data-driven solutions
- **Interests**: Financial infrastructure, blockchain technology, data analytics

## Previous Experience
- **Diebold Nixdorf**: Software Developer working on banking and payments applications
- [NOTE: Add more companies/roles as you share them]

## Technical Skills
- **Languages**: [TO BE FILLED - e.g., JavaScript, Python, etc.]
- **Frameworks**: [TO BE FILLED - e.g., React, Next.js, etc.]
- **Domains**: FinTech, Web3, Data Engineering, Backend Systems
- **Tools**: [TO BE FILLED]

## Notable Projects
[TO BE FILLED - Add 2-3 standout projects you want to highlight]
- Project 1: [Description]
- Project 2: [Description]
- Project 3: [Description]

## Collaboration & Interests
- **Open to**: Partnerships and collaborations
- **Particularly Interested In**: [TO BE FILLED - specific types of projects/partnerships]
- **Not Looking For**: Traditional full-time job offers (currently employed)

## Contact Information
- **GitHub**: github.com/mattkikuchi [Verify this is correct]
- **LinkedIn**: linkedin.com/in/mattkikuchi [Verify this is correct]
- **Email**: [TO BE FILLED]
- **Location**: Toronto, Canada

## Personal Interests
- Gaming (original spark for programming)
- Sports & Statistics
- [TO BE FILLED - other hobbies/interests]

## Achievements & Recognition
[TO BE FILLED - Add any notable achievements, awards, open source contributions, etc.]

---
INSTRUCTIONS FOR UPDATING:
1. Replace [TO BE FILLED] sections with actual information
2. Add specific technologies, projects, and achievements
3. Update contact information
4. Keep information current and accurate
5. Remove this instructions section once filled out
`
